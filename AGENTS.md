...
**任何一次代码修改，都要：**
1. 在根目录 `VERSION.md` 顶部新增一条记录，写清本次改了什么（日期用绝对日期）。
2. 升级版本号，并同步更新 `src/frontend/src/version.js` 中的 `APP_VERSION`（顶栏会显示它）。

**版本号格式：`VX.Y.Z`，从 `V1.0.0` 开始。**
- 每次修改在最低位 **Z 加 1**。
- **Z 到 9 后进位**：Z 归 0，Y 加 1。例：`V1.0.9` 的下一个版本是 `V1.1.0`。
- **Y 到 9 后再进位**：Y 归 0，X 加 1。例：`V1.9.9` 的下一个版本是 `V2.0.0`。

即三段各自逢十进一：`V1.0.1 → … → V1.0.9 → V1.1.0 → … → V1.9.9 → V2.0.0`。`VERSION.md` 与 `version.js` 的版本号必须保持一致。
